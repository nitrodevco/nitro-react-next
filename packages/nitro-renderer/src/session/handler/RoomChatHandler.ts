...
export class RoomChatHandler extends BaseHandler
{
    ...
    private onRoomUnitChatEvent(event: RoomUnitChatEvent): void
    {
        ...
        EventStore.getState().emit(chatEvent);
    }
    ...
    private onRoomUnitHandItemReceivedEvent(event: RoomUnitHandItemReceivedEvent): void
    {
        ...
        EventStore.getState().emit(new RoomSessionChatEvent(RoomSessionChatEvent.CHAT_EVENT, session, parser.giverUserId, '', RoomSessionChatEvent.CHAT_TYPE_HAND_ITEM_RECEIVED, SystemChatStyleEnum.GENERIC, [], parser.handItemType));
    }
    ...
    private onRespectReceivedEvent(event: RespectReceivedEvent): void
    {
        ...
        EventStore.getState().emit(new RoomSessionChatEvent(RoomSessionChatEvent.CHAT_EVENT, session, userData.roomIndex, '', RoomSessionChatEvent.CHAT_TYPE_RESPECT, SystemChatStyleEnum.GENERIC));
    }
    ...
    private onPetRespectNoficationEvent(event: PetRespectNoficationEvent): void
    {
        ...
        EventStore.getState().emit(new RoomSessionChatEvent(RoomSessionChatEvent.CHAT_EVENT, session, petData.roomIndex, '', chatType, SystemChatStyleEnum.GENERIC));
    }
    ...
    private onPetSupplementedNotificationEvent(event: PetSupplementedNotificationEvent): void
    {
        ...
        EventStore.getState().emit(new RoomSessionChatEvent(RoomSessionChatEvent.CHAT_EVENT, session, petData.roomIndex, '', chatType, SystemChatStyleEnum.GENERIC, null, userRoomIndex));
    }
    ...
    private onFloodControlEvent(event: FloodControlEvent): void
    {
        ...
        EventStore.getState().emit(new RoomSessionChatEvent(RoomSessionChatEvent.FLOOD_EVENT, session, -1, seconds.toString(), 0, 0));
    }
    ...
    private onRemainingMuteEvent(event: RemainingMuteEvent): void
    {
        ...
        EventStore.getState().emit(new RoomSessionChatEvent(RoomSessionChatEvent.CHAT_EVENT, session, session.ownRoomIndex, '', RoomSessionChatEvent.CHAT_TYPE_MUTE_REMAINING, SystemChatStyleEnum.GENERIC, [], parser.seconds));
    }
}
