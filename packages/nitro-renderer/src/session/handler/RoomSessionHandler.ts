...
export class RoomSessionHandler extends BaseHandler
{
    ...
    private onRoomDoorbellAcceptedEvent(event: RoomDoorbellAcceptedEvent): void
    {
        ...
        else
        {
            ...
            EventStore.getState().emit(new RoomSessionDoorbellEvent(RoomSessionDoorbellEvent.RSDE_ACCEPTED, session, username));
        }
    }
    ...
    private onRoomDoorbellRejectedEvent(event: FlatAccessDeniedMessageEvent): void
    {
        ...
        else
        {
            ...
            EventStore.getState().emit(new RoomSessionDoorbellEvent(RoomSessionDoorbellEvent.RSDE_REJECTED, session, username));
        }
    }
    ...
    private onYouAreSpectatorMessageEvent(event: YouAreSpectatorMessageEvent): void
    {
        if (this.listener)
        {
            ...
            EventStore.getState().emit(new RoomSessionSpectatorModeEvent(RoomSessionSpectatorModeEvent.SPECTATOR_MODE, session));
        }
    }
}
