...
export class SoundManager implements ISoundManager
{
    ...
    public async init(): Promise<void>
    {
        ...
        EventStore.getState().subscribe<RoomEngineSamplePlaybackEvent>(RoomEngineSamplePlaybackEvent.PLAY_SAMPLE, event => this.onEvent(event));
        EventStore.getState().subscribe<RoomEngineObjectEvent>(RoomEngineObjectEvent.REMOVED, event => this.onEvent(event));
        EventStore.getState().subscribe<RoomEngineEvent>(RoomEngineEvent.DISPOSED, event => this.onEvent(event));
        EventStore.getState().subscribe<NitroSettingsEvent>(NitroSettingsEvent.SETTINGS_UPDATED, event => this.onEvent(event));
        EventStore.getState().subscribe<NitroSoundEvent>(NitroSoundEvent.PLAY_SOUND, event => this.onEvent(event));
    }
