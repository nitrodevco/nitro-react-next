...
export class MusicController implements IMusicController
{
    ...
    public init(): void
    {
        ...
        this._subscriptions.push(
            ...
            EventStore.getState().subscribe(RoomObjectSoundMachineEvent.JUKEBOX_DISPOSE, this.onJukeboxDispose),
            EventStore.getState().subscribe(RoomObjectSoundMachineEvent.SOUND_MACHINE_INIT, this.onSoundMachineInit),
            EventStore.getState().subscribe(RoomObjectSoundMachineEvent.SOUND_MACHINE_DISPOSE, this.onSoundMachineDispose),
            EventStore.getState().subscribe(SoundManagerEvent.TRAX_SONG_COMPLETE, this.onTraxSongComplete)
        );
    }
    ...
    public dispose(): void
    {
        ...
        this._subscriptions.splice(0)?.forEach(unsubscribe => unsubscribe());
    }
    ...
    protected onTraxSongComplete(k: SoundManagerEvent): void
    {
        if (this.getSongIdPlayingAtPriority(this._priorityPlaying) === k.id)
        {
            ...
            {
                EventStore.getState().emit(new NowPlayingEvent(NowPlayingEvent.NPW_USER_STOP_SONG, priorityPlaying, k.id, -1));
            }
        }
    }
    ...
    private onTraxSongInfoMessageEvent(event: TraxSongInfoMessageEvent): void
    {
        ...
        for (const song of parser.songs)
        {
            ...
            if (!songAvailable)
            {
                ...
                EventStore.getState().emit(new SongInfoReceivedEvent(SongInfoReceivedEvent.SIR_TRAX_SONG_INFO_RECEIVED, song.id));
                ...
                while (this._diskInventoryMissingData.indexOf(song.id) != -1)
                {
                    ...
                    {
                        EventStore.getState().emit(new SongDiskInventoryReceivedEvent(SongDiskInventoryReceivedEvent.SDIR_SONG_DISK_INVENTORY_RECEIVENT_EVENT));
                    }
                }
    ...
    private playSongObject(priority: number, songId: number): boolean
    {
        ...
        */
        ...
        {
            EventStore.getState().emit(new NowPlayingEvent(NowPlayingEvent.NPE_USER_PLAY_SONG, priority, songData.id, -1));
        }
        return true;
    }
    ...
    private notifySongPlaying(k: SongDataEntry): void
    {
        ...
        {
            EventStore.getState().emit(new NotifyPlayedSongEvent(k.name, k.creator));
