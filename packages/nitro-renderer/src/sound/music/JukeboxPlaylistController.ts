...
export class JukeboxPlaylistController implements IPlaylistController
{
    ...
    public init(): void
    {
        ...
        this._subscriptions.push(
            EventStore.getState().subscribe(SongInfoReceivedEvent.SIR_TRAX_SONG_INFO_RECEIVED, this.onSongInfoReceivedEvent)
        );
    }
    ...
    private onNowPlayingMessageEvent(event: NowPlayingMessageEvent): void
    {
        ...
        EventStore.getState().emit(new NowPlayingEvent(NowPlayingEvent.NPE_SONG_CHANGED, MusicPriorities.PRIORITY_ROOM_PLAYLIST, parser.currentSongId, parser.currentPosition));
    }
    ...
    private onJukeboxSongDisksMessageEvent(event: JukeboxSongDisksMessageEvent): void
    {
        ...
        if (!this._missingSongInfo.length) EventStore.getState().emit(new PlayListStatusEvent(PlayListStatusEvent.PLUE_PLAY_LIST_UPDATED));
    }

    private onJukeboxPlayListFullMessageEvent(event: JukeboxPlayListFullMessageEvent): void
    {
        EventStore.getState().emit(new PlayListStatusEvent(PlayListStatusEvent.PLUE_PLAY_LIST_FULL));
    }
