...
export class EffectAssetDownloadManager
{
    ...
    public async init(): Promise<void>
    {
        ...
        EventStore.getState().subscribe(NitroEventType.AVATAR_EFFECT_DOWNLOADED, (event: AvatarRenderEffectLibraryEvent) => this.onLibraryLoaded(event));
    ...
    private onLibraryLoaded(event: AvatarRenderEffectLibraryEvent): void
    {
        ...
        for (const [id, libraries] of this._incompleteEffects.entries())
        {
            ...
            if (isReady)
            {
                ...
                EventStore.getState().emit(new NitroEvent(NitroEventType.AVATAR_EFFECT_LOADED));
            }
        }
