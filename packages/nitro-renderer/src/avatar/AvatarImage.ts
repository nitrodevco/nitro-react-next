import { AvatarAction, AvatarDirectionAngle, AvatarScaleType, AvatarSetType, IActiveActionData, IAnimationLayerData, IAvatarDataContainer, IAvatarEffectListener, IAvatarFigureContainer, IAvatarImage, IPartColor, ISpriteDataContainer } from '#renderer/api';
import { GetGrayscaleFilter, GetRenderer, GetTexturePool, GetTickerTime, PaletteMapFilter, TextureUtils } from '#renderer/utils';
...
export class AvatarImage implements IAvatarImage, IAvatarEffectListener
{
    ...
    private buildAvatarContainer(avatarCanvas: AvatarCanvas, setType: string): Container
    {
        ...
        if (this._avatarSpriteData)
        {
            ...
            {
                container.filters = [...Array.from(container.filters), this._avatarSpriteData.colorTransform];
            }

            if (this._avatarSpriteData.paletteIsGrayscale)
            {
                container.filters = [...Array.from(container.filters), GetGrayscaleFilter(), new PaletteMapFilter({
    ...
    public processAsTexture(setType: string, hightlight: boolean = false): Texture
    {
