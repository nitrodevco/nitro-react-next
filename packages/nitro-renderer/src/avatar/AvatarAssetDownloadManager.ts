...
export class AvatarAssetDownloadManager
{
    ...
    public async init(): Promise<void>
    {
        ...
        EventStore.getState().subscribe<AvatarRenderLibraryEvent>(NitroEventType.AVATAR_ASSET_DOWNLOADED, event => this.onLibraryLoaded(event));
    ...
    private onLibraryLoaded(event: AvatarRenderLibraryEvent): void
    {
        ...
        for (const [figure, libraries] of this._incompleteFigures.entries())
        {
            ...
            if (isReady)
            {
                ...
                EventStore.getState().emit(new NitroEvent(NitroEventType.AVATAR_ASSET_LOADED));
            }
        }
