...
export class SocketConnection implements IConnection
{
    ...
    public init(socketUrl: string): void
    {
        ...
        this._socket.addEventListener(WebSocketEventEnum.CONNECTION_OPENED, event => EventStore.getState().emit(new NitroEvent(NitroEventType.SOCKET_OPENED)));

        this._socket.addEventListener(WebSocketEventEnum.CONNECTION_CLOSED, event => EventStore.getState().emit(new NitroEvent(NitroEventType.SOCKET_CLOSED)));

        this._socket.addEventListener(WebSocketEventEnum.CONNECTION_ERROR, event => EventStore.getState().emit(new NitroEvent(NitroEventType.SOCKET_ERROR)));
