...
export class RoomEngine implements IRoomEngine, IRoomCreator, IRoomEngineServices, IRoomManagerListener, IRoomContentListener, IUpdateReceiver
{
    ...
    public async init(): Promise<void>
    {
        ...
        EventStore.getState().subscribe<RoomSessionEvent>(RoomSessionEvent.STARTED, event => this.onRoomSessionEvent(event));
        EventStore.getState().subscribe<RoomSessionEvent>(RoomSessionEvent.ENDED, event => this.onRoomSessionEvent(event));
    ...
    public removeRoomInstance(roomId: number): void
    {
        ...
        EventStore.getState().emit(new RoomEngineEvent(RoomEngineEvent.DISPOSED, roomId));
    }
    ...
    public createRoomInstance(roomId: number, roomMap: RoomMapData): void
    {
        ...
        EventStore.getState().emit(new RoomEngineEvent(RoomEngineEvent.INITIALIZED, roomId));
    }
    ...
    public setRoomInstanceRenderingCanvasScale(roomId: number, canvasId: number, level: number, point: Point = null, offsetPoint: Point = null, isFlipForced: boolean = false, flag: boolean = false): void
    {
        ...
        EventStore.getState().emit(new RoomEngineEvent(RoomEngineEvent.ROOM_ZOOMED, roomId));
    }
    ...
    public setRoomInstanceRenderingCanvasOffset(roomId: number, canvasId: number, point: Point): boolean
    {
        ...
        EventStore.getState().emit(new RoomDragEvent(roomId, -(renderingCanvas.screenOffsetX - x), -(renderingCanvas.screenOffsetY - y)));
    ...
    public updateAreaHide(roomId: number, furniId: number, on: boolean, rootX: number, rootY: number, width: number, length: number, invert: boolean): boolean
    {
        ...
        EventStore.getState().emit(new RoomEngineAreaHideStateEvent(roomId, furniId, RoomObjectCategory.FLOOR, on));
    ...
    public updateObjectRoomColor(roomId: number, color: number, light: number, backgroundOnly: boolean): boolean
    {
        ...
        EventStore.getState().emit(new RoomBackgroundColorEvent(roomId, color, light, backgroundOnly));
    ...
    public setRoomEngineGameMode(roomId: number, isPlaying: boolean): void
    {
        ...
        if (mode === 0)
        {
            EventStore.getState().emit(new RoomEngineEvent(RoomEngineEvent.NORMAL_MODE, roomId));
        }
        else
        {
            EventStore.getState().emit(new RoomEngineEvent(RoomEngineEvent.GAME_MODE, roomId));
        }
    }
    ...
    private addObjectWallItemFromData(roomId: number, id: number, data: RoomFurnitureData): boolean
    {
        ...
        EventStore.getState().emit(new RoomEngineObjectEvent(RoomEngineObjectEvent.ADDED, roomId, id, RoomObjectCategory.WALL));
    ...
    private removeRoomObject(roomId: number, objectId: number, category: number): void
    {
        ...
        EventStore.getState().emit(new RoomEngineObjectEvent(RoomEngineObjectEvent.REMOVED, roomId, objectId, category));
    }
    ...
    public addRoomObjectUser(roomId: number, objectId: number, location: IVector3D, direction: IVector3D, headDirection: number, type: number, figure: string): boolean
    {
        ...
        EventStore.getState().emit(new RoomEngineObjectEvent(RoomEngineObjectEvent.ADDED, roomId, objectId, RoomObjectCategory.UNIT));
    ...
    public updateRoomObjectUserLocation(roomId: number, objectId: number, location: IVector3D, targetLocation: IVector3D, canStandUp: boolean = false, baseY: number = 0, direction: IVector3D = null, headDirection: number = NaN): boolean
    {
        ...
        {
            EventStore.getState().emit(new RoomToObjectOwnAvatarMoveEvent(RoomToObjectOwnAvatarMoveEvent.ROAME_MOVE_TO, targetLocation));
        }
    ...
    public loadRoomObjectBadgeImage(roomId: number, objectId: number, objectCategory: number, badgeId: string, groupBadge: boolean = true): void
    {
        ...
        if (!badgeName)
        {
            ...
            if (!this._badgeListenerObjects.size)
            {
                ...
                EventStore.getState().subscribe(BadgeImageReadyEvent.IMAGE_READY, onBadgeImageReadyEvent);
            }
        ...
        else
        {
            if (!this.handleRoomDragging(canvas, x, y, type, altKey, ctrlKey, shiftKey))
            {
                if (!canvas.handleMouseEvent(x, y, type, altKey, ctrlKey, shiftKey, buttonDown))
                {
                    ...
                    {
                        EventStore.getState().emit(new RoomEngineObjectEvent(RoomEngineObjectEvent.DESELECTED, this._activeRoomId, -1, RoomObjectCategory.MINIMUM));
    ...
    private removeOverlayIconSprite(container: Container, label: string): boolean
    {
        ...
        while (index >= 0)
        {
            const child = (container.getChildAt(index)) as Container;
            ...
            if (child)
            {
                if (child.label === label)
                {
                    ...
                    {
                        const firstChild = (child.getChildAt(0)) as Container;
    ...
    public objectsInitialized(k: string): void
    {
        ...
        EventStore.getState().emit(new RoomEngineEvent(RoomEngineEvent.OBJECTS_INITIALIZED, roomId));
    }
