import { IGetImageListener, IImageResult, IObjectData, IRoomEngine, IRoomObjectController, IRoomRenderingCanvas, IVector3D, LegacyDataType, RoomObjectCategory, RoomObjectUserType, RoomObjectVariable } from '#renderer/api';
...
export class RoomPreviewer
{
    ...
    constructor(roomEngine: IRoomEngine, roomId: number = 1)
    {
        ...
        this._subscriptions.push(
            EventStore.getState().subscribe(RoomEngineObjectEvent.ADDED, this.onRoomObjectAdded),
            EventStore.getState().subscribe(RoomEngineObjectEvent.CONTENT_UPDATED, this.onRoomObjectAdded),
            EventStore.getState().subscribe(RoomEngineEvent.INITIALIZED, this.onRoomInitializedonRoomInitialized)
        );
