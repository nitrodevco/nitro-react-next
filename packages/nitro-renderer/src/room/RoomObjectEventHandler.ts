...
import { CreateLinkEvent, RoomId, Vector3d } from '#renderer/utils';
...
export class RoomObjectEventHandler implements IRoomCanvasMouseListener, IRoomObjectEventManager
{
    ...
    {
        EventStore.getState().subscribe<RoomEngineObjectEvent>(RoomEngineObjectEvent.ADDED, event => this.onRoomEngineObjectEvent(event));
    }
            ...
            case RoomObjectOperationType.OBJECT_UNDEFINED:
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        EventStore.getState().emit(new RoomEngineObjectEvent(RoomEngineObjectEvent.DESELECTED, roomId, -1, RoomObjectCategory.MINIMUM));
                    }
                    ...
                    else if ((category === RoomObjectCategory.FLOOR) || (category === RoomObjectCategory.WALL))
                    {
                        if (event.altKey || event.ctrlKey || event.shiftKey)
                        {
                            if (!event.ctrlKey && !event.altKey && event.shiftKey)
                            {
                                if (category === RoomObjectCategory.FLOOR)
                                {
                                    EventStore.getState().emit(new RoomEngineObjectEvent(RoomEngineObjectEvent.REQUEST_ROTATE, roomId, event.objectId, category));
                                }
                            }
    ...
    private handleRoomObjectMouseDoubleClickEvent(event: RoomObjectMouseEvent, roomId: number): void
    {
        ...
        EventStore.getState().emit(new RoomEngineObjectEvent(RoomEngineObjectEvent.DOUBLE_CLICK, roomId, id, category));
    }
        ...
        switch (operation)
        {
            case RoomObjectOperationType.OBJECT_UNDEFINED:
                if ((category === RoomObjectCategory.FLOOR) || (category === RoomObjectCategory.WALL) || (event.objectType === RoomObjectUserType.MONSTER_PLANT))
                {
                    ...
                    {
                        EventStore.getState().emit(new RoomEngineObjectEvent(RoomEngineObjectEvent.REQUEST_MOVE, roomId, event.objectId, category));
                    }
                }
                return;
        }
        ...
        switch (operation)
        {
            case RoomObjectOperationType.OBJECT_UNDEFINED:
                if ((category === RoomObjectCategory.FLOOR) || (category === RoomObjectCategory.WALL) || (event.objectType === RoomObjectUserType.MONSTER_PLANT))
                {
                    ...
                    {
                        EventStore.getState().emit(new RoomEngineObjectEvent(RoomEngineObjectEvent.REQUEST_MANIPULATION, roomId, event.objectId, category));
                    }
                }
                return;
        }
    ...
    private handleRoomObjectMouseEnterEvent(event: RoomObjectMouseEvent, roomId: number): void
    {
        ...
        EventStore.getState().emit(new RoomEngineObjectEvent(RoomEngineObjectEvent.MOUSE_ENTER, roomId, id, category));
    }
    ...
    private handleRoomObjectMouseLeaveEvent(event: RoomObjectMouseEvent, roomId: number): void
    {
        ...
        EventStore.getState().emit(new RoomEngineObjectEvent(RoomEngineObjectEvent.MOUSE_LEAVE, roomId, id, category));
    }
    ...
    private onRoomObjectDimmerStateUpdateEvent(event: RoomObjectDimmerStateUpdateEvent, roomId: number): void
    {
        ...
        {
            case RoomObjectDimmerStateUpdateEvent.DIMMER_STATE:
                EventStore.getState().emit(new RoomEngineDimmerStateEvent(roomId, event.state, event.presetId, event.effectId, event.color, event.brightness));
                return;
        }
    }
    ...
    private onRoomObjectWidgetRequestEvent(event: RoomObjectWidgetRequestEvent, roomId: number): void
    {
        ...
        {
            case RoomObjectWidgetRequestEvent.OPEN_WIDGET:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.OPEN_WIDGET, roomId, objectId, objectCategory, ((event.object as IRoomObjectController).logic.widget)));
                return;
            case RoomObjectWidgetRequestEvent.CLOSE_WIDGET:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.CLOSE_WIDGET, roomId, objectId, objectCategory, ((event.object as IRoomObjectController).logic.widget)));
                return;
            case RoomObjectWidgetRequestEvent.OPEN_FURNI_CONTEXT_MENU:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.OPEN_FURNI_CONTEXT_MENU, roomId, objectId, objectCategory, ((event.object as IRoomObjectController).logic.contextMenu)));
                return;
            case RoomObjectWidgetRequestEvent.CLOSE_FURNI_CONTEXT_MENU:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.CLOSE_FURNI_CONTEXT_MENU, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.PLACEHOLDER:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_PLACEHOLDER, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.CREDITFURNI:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_CREDITFURNI, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.STACK_HEIGHT:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_STACK_HEIGHT, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.EXTERNAL_IMAGE:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_EXTERNAL_IMAGE, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.STICKIE:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_STICKIE, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.PRESENT:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_PRESENT, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.TROPHY:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_TROPHY, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.TEASER:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_TEASER, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.ECOTRONBOX:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_ECOTRONBOX, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.DIMMER:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_DIMMER, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.WIDGET_REMOVE_DIMMER:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REMOVE_DIMMER, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.CLOTHING_CHANGE:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_CLOTHING_CHANGE, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.JUKEBOX_PLAYLIST_EDITOR:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_PLAYLIST_EDITOR, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.MANNEQUIN:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_MANNEQUIN, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.PET_PRODUCT_MENU:
                EventStore.getState().emit(new RoomEngineUseProductEvent(RoomEngineUseProductEvent.USE_PRODUCT_FROM_ROOM, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.GUILD_FURNI_CONTEXT_MENU:
                ...
                return;
            case RoomObjectWidgetRequestEvent.MONSTERPLANT_SEED_PLANT_CONFIRMATION_DIALOG:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_MONSTERPLANT_SEED_PLANT_CONFIRMATION_DIALOG, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.PURCHASABLE_CLOTHING_CONFIRMATION_DIALOG:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_PURCHASABLE_CLOTHING_CONFIRMATION_DIALOG, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.BACKGROUND_COLOR:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_BACKGROUND_COLOR, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.AREA_HIDE:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_AREA_HIDE, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.MYSTERYBOX_OPEN_DIALOG:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_MYSTERYBOX_OPEN_DIALOG, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.EFFECTBOX_OPEN_DIALOG:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_EFFECTBOX_OPEN_DIALOG, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.MYSTERYTROPHY_OPEN_DIALOG:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_MYSTERYTROPHY_OPEN_DIALOG, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.ACHIEVEMENT_RESOLUTION_OPEN:
                GetCommunication().connection.send(new GetResolutionAchievementsMessageComposer(event.objectId, 0));
                return;
            case RoomObjectWidgetRequestEvent.ACHIEVEMENT_RESOLUTION_ENGRAVING:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_ACHIEVEMENT_RESOLUTION_ENGRAVING, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.ACHIEVEMENT_RESOLUTION_FAILED:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_ACHIEVEMENT_RESOLUTION_FAILED, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.FRIEND_FURNITURE_CONFIRM:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_FRIEND_FURNITURE_CONFIRM, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.FRIEND_FURNITURE_ENGRAVING:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_FRIEND_FURNITURE_ENGRAVING, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.BADGE_DISPLAY_ENGRAVING:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_BADGE_DISPLAY_ENGRAVING, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.HIGH_SCORE_DISPLAY:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_HIGH_SCORE_DISPLAY, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.HIDE_HIGH_SCORE_DISPLAY:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_HIDE_HIGH_SCORE_DISPLAY, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.INERNAL_LINK:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_INTERNAL_LINK, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.ROOM_LINK:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_ROOM_LINK, roomId, objectId, objectCategory));
                return;
            case RoomObjectWidgetRequestEvent.YOUTUBE:
                EventStore.getState().emit(new RoomEngineTriggerWidgetEvent(RoomEngineTriggerWidgetEvent.REQUEST_YOUTUBE, roomId, objectId, objectCategory));
                return;
        }
    }
    ...
    private handleObjectSoundMachineEvent(event: RoomObjectEvent, roomId: number): void
    {
        ...
        {
            case RoomObjectFurnitureActionEvent.SOUND_MACHINE_INIT:
                EventStore.getState().emit(new RoomObjectSoundMachineEvent(RoomObjectSoundMachineEvent.SOUND_MACHINE_INIT, roomId, event.objectId, objectCategory));
                return;
            case RoomObjectFurnitureActionEvent.SOUND_MACHINE_START:
                EventStore.getState().emit(new RoomObjectSoundMachineEvent(RoomObjectSoundMachineEvent.SOUND_MACHINE_SWITCHED_ON, roomId, event.objectId, objectCategory));
                return;
            case RoomObjectFurnitureActionEvent.SOUND_MACHINE_STOP:
                EventStore.getState().emit(new RoomObjectSoundMachineEvent(RoomObjectSoundMachineEvent.SOUND_MACHINE_SWITCHED_OFF, roomId, event.objectId, objectCategory));
                return;
            case RoomObjectFurnitureActionEvent.SOUND_MACHINE_DISPOSE:
                EventStore.getState().emit(new RoomObjectSoundMachineEvent(RoomObjectSoundMachineEvent.SOUND_MACHINE_DISPOSE, roomId, event.objectId, objectCategory));
                return;
        }
    }
    ...
    private handleObjectJukeboxEvent(event: RoomObjectEvent, roomId: number): void
    {
        ...
        {
            case RoomObjectFurnitureActionEvent.JUKEBOX_INIT:
                EventStore.getState().emit(new RoomObjectSoundMachineEvent(RoomObjectSoundMachineEvent.JUKEBOX_INIT, roomId, event.objectId, objectCategory));
                return;
            case RoomObjectFurnitureActionEvent.JUKEBOX_START:
                EventStore.getState().emit(new RoomObjectSoundMachineEvent(RoomObjectSoundMachineEvent.JUKEBOX_SWITCHED_ON, roomId, event.objectId, objectCategory));
                return;
            case RoomObjectFurnitureActionEvent.JUKEBOX_MACHINE_STOP:
                EventStore.getState().emit(new RoomObjectSoundMachineEvent(RoomObjectSoundMachineEvent.JUKEBOX_SWITCHED_OFF, roomId, event.objectId, objectCategory));
                return;
            case RoomObjectFurnitureActionEvent.JUKEBOX_DISPOSE:
                EventStore.getState().emit(new RoomObjectSoundMachineEvent(RoomObjectSoundMachineEvent.JUKEBOX_DISPOSE, roomId, event.objectId, objectCategory));
                return;
        }
    }
    ...
    private onRoomObjectRoomAdEvent(event: RoomObjectRoomAdEvent, roomId: number): void
    {
        ...
        if (eventType) EventStore.getState().emit(new RoomEngineObjectEvent(eventType, roomId, event.objectId, this._roomEngine.getRoomObjectCategoryForType(event.objectType)));
    }
    ...
    private handleRoomObjectPlaySoundEvent(event: RoomObjectPlaySoundIdEvent, roomId: number): void
    {
        ...
        {
            case RoomObjectPlaySoundIdEvent.PLAY_SOUND:
                EventStore.getState().emit(new RoomEngineObjectPlaySoundEvent(RoomEngineObjectPlaySoundEvent.PLAY_SOUND, roomId, event.objectId, objectCategory, event.soundId, event.pitch));
                return;
            case RoomObjectPlaySoundIdEvent.PLAY_SOUND_AT_PITCH:
                EventStore.getState().emit(new RoomEngineObjectPlaySoundEvent(RoomEngineObjectPlaySoundEvent.PLAY_SOUND_AT_PITCH, roomId, event.objectId, objectCategory, event.soundId, event.pitch));
                return;
        }
    }
    ...
    private handleRoomObjectSamplePlaybackEvent(event: RoomObjectSamplePlaybackEvent, roomId: number): void
    {
        ...
        {
            case RoomObjectSamplePlaybackEvent.ROOM_OBJECT_INITIALIZED:
                EventStore.getState().emit(new RoomEngineSamplePlaybackEvent(RoomEngineSamplePlaybackEvent.ROOM_OBJECT_INITIALIZED, roomId, event.objectId, objectCategory, event.sampleId, event.pitch));
                break;
            case RoomObjectSamplePlaybackEvent.ROOM_OBJECT_DISPOSED:
                EventStore.getState().emit(new RoomEngineSamplePlaybackEvent(RoomEngineSamplePlaybackEvent.ROOM_OBJECT_DISPOSED, roomId, event.objectId, objectCategory, event.sampleId, event.pitch));
                break;
            case RoomObjectSamplePlaybackEvent.PLAY_SAMPLE:
                EventStore.getState().emit(new RoomEngineSamplePlaybackEvent(RoomEngineSamplePlaybackEvent.PLAY_SAMPLE, roomId, event.objectId, objectCategory, event.sampleId, event.pitch));
                break;
            case RoomObjectSamplePlaybackEvent.CHANGE_PITCH:
                EventStore.getState().emit(new RoomEngineSamplePlaybackEvent(RoomEngineSamplePlaybackEvent.CHANGE_PITCH, roomId, event.objectId, objectCategory, event.sampleId, event.pitch));
                break;
        }
    }
    ...
    private onHSLColorEnableEvent(event: RoomObjectHSLColorEnableEvent, roomId: number): void
    {
        ...
        {
            case RoomObjectHSLColorEnableEvent.ROOM_BACKGROUND_COLOR:
                EventStore.getState().emit(new RoomObjectHSLColorEnabledEvent(RoomObjectHSLColorEnabledEvent.ROOM_BACKGROUND_COLOR, roomId, event.enable, event.hue, event.saturation, event.lightness));
                return;
        }
    }
    ...
    public setSelectedObject(roomId: number, objectId: number, category: number): void
    {
        ...
        switch (category)
        {
            ...
            case RoomObjectCategory.WALL:
                ...
                EventStore.getState().emit(new RoomEngineObjectEvent(RoomEngineObjectEvent.SELECTED, roomId, objectId, category));
